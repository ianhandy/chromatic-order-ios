...
struct ColorPickerSheet: View {
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 18) {
                ...
                // three numeric sliders. In shift mode this becomes a
                // 5-cell progression strip that shows the start color
                // stepped by (ΔL, Δc, Δh) so the player can preview
                // what the gradient will actually look like.
                previewStrip
                    .frame(height: 140)
                    .overlay(
    ...
    /// Preview shown above the sliders. In absolute-color mode, a
    /// single solid swatch. In shift mode, a horizontal five-cell
    /// gradient strip generated by stepping the start color by
    /// (ΔL, Δc, Δh) — same preview trajectory the ShiftChip uses on
    /// the creator canvas, so the two surfaces stay consistent.
    @ViewBuilder
    private var previewStrip: some View {
        if isNilled {
            HStack(spacing: 3) {
                ForEach(0..<5, id: \.self) { i in
                    let stepped = shiftedColor(step: i)
                    Rectangle()
    ...
    /// OKLCh color at step `i` of the shift-mode progression. Step 0
    /// is the anchor (start) color; each subsequent step advances by
    /// the (ΔL, Δc, Δh) deltas. Mirrors CreatorState's single-anchor
    /// shift-mode interpolation.
    private func shiftedColor(step i: Int) -> OKLCh {
        let t = Double(i)
        return OKLCh(
            L: color.L + deltaL * t,
            c: color.c + deltaC * t,
            h: OK.normH(color.h + deltaH * t)
        )
    }
}
